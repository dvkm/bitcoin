...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ShrimpTempura";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ShrimpTempura";
#else
    // Unix
    return pathRet / ".shrimptempura";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("ShrimpTempura") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Shrimp Tempura developers";
    }
